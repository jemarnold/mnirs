...
**v0.6.2 \| R \| MIT** — workflow/dependency map. See `README.md` +
...
| `interval_times`  | numeric          | set by [`extract_intervals()`](https://jemarnold.github.io/mnirs/reference/extract_intervals.md)       |
| `interval_span`   | numeric(2)       | span used in [`extract_intervals()`](https://jemarnold.github.io/mnirs/reference/extract_intervals.md) |

`verbose` read from `getOption("mnirs.verbose", TRUE)` or pass
explicitly. Access: `attr(data, "nirs_channels")`.
...
**Order rules:** - `resample` → `replace`: window replacement needs
regular `time_channel` - `replace` → `filter`: `"smooth_spline"` /
`"butterworth"` error on internal `NA` (unless `na.rm = TRUE`) -
`resample` → `filter("smooth_spline")`: errors on duplicated time
values - `resample` → `extract_intervals(event_groups = "ensemble")`:
ensemble needs regular samples - `extract_intervals` →
`analyse_kinetics` for multiple intervals
...
`spar` (NULL = GCV auto); errors on NA/duplicated time.

**`"butterworth"`** (requires
[signal](https://signal.R-forge.R-project.org)): `order` (default `2L`),
`W`/`fc` (normalised or Hz), `type` (`"low"`, `"high"`, `"stop"`,
`"pass"`), `edges` (`"rev"`, `"rep1"`, `"none"`); errors on NA.

**`"moving_average"`**: `width` (samples) or `span` (time units);
...
- `start = by_lap(), end = by_lap()`: first start sample → last end
  sample
- `span = c(before, after)` recycled to boundaries; negative = before
  start, positive = after end
- `span = list()` recycled to intervals
- `event_groups = "distinct"`: one df per event
- `event_groups = "ensemble"`: single ensemble-averaged df (requires
  regular time grid)
...
**Input:** single df → `"interval_1"`; named list → each separate;
grouped df → split by group.

**Per-method args `...`:**

`"half_response_time"`: `t0` (default `0`), `fraction` (default `0.5`;
`0.632` ≈ MRT)

`"peak_slope"`: `width` or `span` (one required); `align` (`"centre"`,
`"left"`, `"right"`); `partial` (default `FALSE`)

`"monoexponential"`: `time_delay` (default `TRUE`; 4-param → 3-param
fallback); accepts [`stats::nls()`](https://rdrr.io/r/stats/nls.html)
args
...
Names must match `nirs_channels` exactly; unrecognised names silently
ignored.

**Coefficients:**

`"half_response_time"`: `interval`, `nirs_channels`, `time_channel`,
...
`"monoexponential"`: `interval`, `nirs_channels`, `time_channel`, `A`,
`B`, `tau`, `k`, `TD`, `MRT`, `HRT`, `tau_fitted`, `MRT_fitted`,
`HRT_fitted`

**Diagnostics:** `n_obs`, `r2`, `adj_r2`, `pseudo_r2`, `rmse`, `snr`,
and `cv_rmse`
...
| `ggplot2`    | plotting          | Suggests | [`plot.mnirs()`](https://jemarnold.github.io/mnirs/reference/plot.mnirs.md) |
| `scales`     | axis formatting   | Suggests | [`plot.mnirs()`](https://jemarnold.github.io/mnirs/reference/plot.mnirs.md) |
...
| Constraint                                                                                                 | Detail                                                                                                                                               |
|------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|
| [`example_mnirs()`](https://jemarnold.github.io/mnirs/reference/example_mnirs.md) returns path             | pass to `read_mnirs(file_path = example_mnirs(...))`                                                                                                 |
| Irregular samples warning from [`read_mnirs()`](https://jemarnold.github.io/mnirs/reference/read_mnirs.md) | fires in pipe even after downstream [`resample_mnirs()`](https://jemarnold.github.io/mnirs/reference/resample_mnirs.md); verify by inspecting output |
| Pipeline order                                                                                             | `resample → replace → filter`; wrong order = incorrect results                                                                                       |
| `replace_invalid` / `replace_outliers` default `"median"`                                                  | [`replace_mnirs()`](https://jemarnold.github.io/mnirs/reference/replace_mnirs.md) default `"linear"`                                                 |
| `"smooth_spline"` / `"butterworth"` fail on NA                                                             | use [`replace_mnirs()`](https://jemarnold.github.io/mnirs/reference/replace_mnirs.md) first or `na.rm = TRUE`                                        |
| `"smooth_spline"` fails on duplicated time                                                                 | use [`resample_mnirs()`](https://jemarnold.github.io/mnirs/reference/resample_mnirs.md) first                                                        |
...
| File                          | Contents                                                                                                                                                                                                                                                                                                                                                                                |
|-------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `R/read_mnirs.R`              | [`read_mnirs()`](https://jemarnold.github.io/mnirs/reference/read_mnirs.md), [`example_mnirs()`](https://jemarnold.github.io/mnirs/reference/example_mnirs.md), [`create_mnirs_data()`](https://jemarnold.github.io/mnirs/reference/create_mnirs_data.md)                                                                                                                               |
| `R/resample_mnirs.R`          | [`resample_mnirs()`](https://jemarnold.github.io/mnirs/reference/resample_mnirs.md)                                                                                                                                                                                                                                                                                                     |
| `R/replace_mnirs.R`           | [`replace_mnirs()`](https://jemarnold.github.io/mnirs/reference/replace_mnirs.md), [`replace_invalid()`](https://jemarnold.github.io/mnirs/reference/replace_mnirs.md), [`replace_outliers()`](https://jemarnold.github.io/mnirs/reference/replace_mnirs.md), [`replace_missing()`](https://jemarnold.github.io/mnirs/reference/replace_mnirs.md)                                       |
| `R/filter_mnirs.R`            | [`filter_mnirs()`](https://jemarnold.github.io/mnirs/reference/filter_mnirs.md), [`filter_ma()`](https://jemarnold.github.io/mnirs/reference/filter_ma.md), [`filter_butter()`](https://jemarnold.github.io/mnirs/reference/filter_butter.md)                                                                                                                                           |
| `R/shift_mnirs.R`             | [`shift_mnirs()`](https://jemarnold.github.io/mnirs/reference/shift_mnirs.md)                                                                                                                                                                                                                                                                                                           |
| `R/rescale_mnirs.R`           | [`rescale_mnirs()`](https://jemarnold.github.io/mnirs/reference/rescale_mnirs.md)                                                                                                                                                                                                                                                                                                       |
| `R/extract_intervals.R`       | [`extract_intervals()`](https://jemarnold.github.io/mnirs/reference/extract_intervals.md), [`by_time()`](https://jemarnold.github.io/mnirs/reference/by_time.md), [`by_label()`](https://jemarnold.github.io/mnirs/reference/by_time.md), [`by_lap()`](https://jemarnold.github.io/mnirs/reference/by_time.md), [`by_sample()`](https://jemarnold.github.io/mnirs/reference/by_time.md) |
...
| `R/plot.mnirs.R`              | [`plot.mnirs()`](https://jemarnold.github.io/mnirs/reference/plot.mnirs.md), [`theme_mnirs()`](https://jemarnold.github.io/mnirs/reference/theme_mnirs.md), [`palette_mnirs()`](https://jemarnold.github.io/mnirs/reference/palette_mnirs.md), scale functions                                                                                                                          |
