...
detect_events <- function(
    ...
    event_indices <- sort(unique(c(
        event_samples,
        findInterval(event_times, time_vec),
        label_indices
    )))
...
recycle_param <- function(param, n_events, group_events, verbose = TRUE) {
    ...
    ## expand span to fit group_events
    ## span = list(c(-0.3, 0.3), c(-0.5, 0.5))
    ## expand span to n events within each group
    ## span = list(c(-0.3, 0.3), c(-0.3, 0.3), c(-0.5, 0.5), c(-0.5, 0.5))

    ## recycle param to match group_events if custom grouping specified
    if (is.numeric(group_events[[1L]]) && n_param < n_groups) {
        n_groups <- length(group_events)
        param_missing <- n_groups - n_param
        ## recycle last param item if fewer params than groups
        param <- param[
            c(seq_len(n_param), rep(n_param, param_missing))
        ]

        if (verbose && n_param > 1) {
            ## only display when more than one param specified
            cli_inform(c(
                "i" = "{.arg {substitute(param)}} recycled to meet \\
                {.val {param_missing}} event group{?s}."
            ))
        }

        ## now have param length == group length
        ## if param length already == n_events and
        ## group_events already in order, then return param as-is
        if (n_param == n_events && !is.unsorted(groups_unlisted)) {
            return(param)
        }

        ## find any ungrouped/unspecified events
        n_ungrouped <- n_events - length(groups_unlisted)
        
        ## ignore extra specified group_events values
        if (n_ungrouped < 0) {
            return(param[seq_len(n_events)])
        }

        ## recycle param to n_events within each group
        param <- param[
            c(rep(seq_along(param), lengths(group_events)), n_ungrouped)
        ][order(groups_unlisted)]
    }
    ...
    if (n_param < n_events) {
        if (verbose && n_param > 1) {
            ...
            cli_inform(c(
                "i" = "{.arg {substitute(param)}} recycled to meet \\
                {.val {n_events - n_param}} unspecified event{?s}."
            ))
        }
        return(param)
    }
    ...
    if (verbose) {
        cli_inform(c(
            ...
            events by {.val {n_param - n_events}}.",
...
extract_interval_list <- function(
    ...
    interval_list <- lapply(n_vec, \(.i) {
        ...
        ## TODO is mnirs metadata necessary here?
        create_mnirs_data(
            interval_data,
...
ensemble_intervals <- function(
    ...
    interval_data <- lapply(interval_list, \(.df) {
        metadata_nirs_channels = attr(.df, "nirs_channels")
    ...
    nirs_device <- attr(interval_list[[1L]], "nirs_device")
    event_channel <- attr(interval_list[[1L]], "event_channel")
    ...
    result <- create_mnirs_data(
        result[order(result[[time_channel]]), ],
...
group_intervals <- function(
    ...
    if (verbose && length(dup) > 0) {
        cli_warn(c(
            "!" = "Duplicates detected of {cli::qty(length(dup))} \\
            interval{?s} {.val {dup}}.",
