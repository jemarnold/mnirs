...
- drift_fraction:
  ...
  onset is held (*default* `0.95`). Always held constant. Applied to
...
- fix:
  ...
  `slope`, `slope_B`) to hold constant during fitting, e.g.
...
`nirs_channels`, `model`, `A`, `B`, `xmid`, `slope`, `texc`, `slope_B`,
`drift_fraction`, `xmid_fitted`, `texc_fitted`. `texc` is the excursion
point where the drift rate overtakes the decaying sigmoid rate, never
before the drift onset (see
[`sigdrift_texc()`](https://jemarnold.github.io/mnirs/reference/sigdrift_texc.md)).
