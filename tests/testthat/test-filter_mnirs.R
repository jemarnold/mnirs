...
test_that("filter_butter validates inputs correctly", {
    expect_error(
        filter_butter(x = "not_numeric", n = 1, W = 0.1),
        "x.*numeric"
    )

    expect_error(
        filter_butter(x = 1:10, n = 0, W = 0.1),
        "n.*integer"
    )

    expect_error(
        filter_butter(x = 1:10, n = -1, W = 0.1),
        "n.*integer"
    )

    expect_error(
        filter_butter(x = 1:10, n = 1.5, W = 0.1),
        "n.*integer"
    )

    expect_error(
        filter_butter(x = 1:10, n = 1, W = 1.5),
        "W.*numeric"
    )

    expect_error(
        filter_butter(x = 1:10, n = 1, W = 0),
        "W.*numeric"
    )

    expect_error(
        filter_butter(x = 1:10, n = 1, W = c(0.1, 0.3), type = "low"),
        "W.*numeric"
    )
})
...
test_that("filter_butter returns correct output structure", {
    x <- rnorm(100)
    result <- filter_butter(x, n = 2, W = 0.1)
...
test_that("filter_butter handles different edge options", {
    ...
    result_none <- filter_butter(x, n = 2, W = 0.1, edges = "none")
    result_rev <- filter_butter(x, n = 2, W = 0.1, edges = "rev")
    result_rep1 <- filter_butter(x, n = 2, W = 0.1, edges = "rep1")
...
test_that("filter_butter handles NA values", {
    ...
    expect_error(
        filter_butter(x_with_na, n = 1, W = 0.1, na.rm = FALSE),
        "x.*NA"
    )

    result <- filter_butter(x_with_na, n = 1, W = 0.1, na.rm = TRUE)
    expect_true(is.na(result[6]))

    # All NA input
    expect_error(
        filter_butter(rep(NA_real_, 5), n = 1, W = 0.1, na.rm = TRUE),
        "x.*valid.*numeric"
    )

    # Leading/trailing NAs
    x_edge_na <- c(NA, NA, 3:8, NA, NA)
    result_edge <- filter_butter(x_edge_na, n = 1, W = 0.1, na.rm = TRUE)
...
test_that("filter_butter handles different filter types", {
    ...
    low <- filter_butter(x, n = 2, W = 0.1, type = "low")
    high <- filter_butter(x, n = 2, W = 0.9, type = "high")
    stop <- filter_butter(x, n = 2, W = c(0.3, 0.7), type = "stop")
    pass <- filter_butter(x, n = 2, W = c(0.3, 0.7), type = "pass")
    ...
    ## wrong W elements
    expect_error(
        filter_butter(x, n = 2, W = c(0.3, 0.7), type = "low"),
        "W.*one-element.*numeric"
    )
    expect_error(
        filter_butter(x, n = 2, W = 0.9, type = "pass"),
...
test_that("filter_butter handles edge cases", {
    # Very short vector
    x_short <- c(1, 2, 3)
    result <- filter_butter(x_short, n = 1, W = 0.5)
    expect_length(result, 3)

    # Single value
    x_single <- 5
    result_single <- filter_butter(x_single, n = 1, W = 0.5)
    expect_length(result_single, 1)

    # Constant signal
    x_const <- rep(10, 50)
    result_const <- filter_butter(x_const, n = 1, W = 0.5)
...
test_that("filter_butter smooths noisy signal", {
    ...
    filtered <- filter_butter(noisy, n = 2, W = 0.2)
...
test_that("smooth_spline handles NAs", {
    ...
    expect_equal(which(is.na(result$smo2_right)),
...
test_that("smooth_spline errors with non-sequential time", {
    moxy_data <- read_mnirs(
        file_path = example_mnirs("moxy_ramp.xlsx"),
        nirs_channels = c(smo2_left = "SmO2 Live",
                          smo2_right = "SmO2 Live(2)"),
...
test_that("butterworth errors without W or fc", {
    expect_error(
        filter_mnirs(moxy_data, method = "butterworth", type = "low", n = 2),
...
test_that("filter_mnirs respects nirs_channels argument", {
    ...
    expect_equal(
        attr(result, "nirs_channels"),  attr(moxy_data, "nirs_channels")
    )
})
...
test_that("filter_mnirs preserves and updates metadata", {
    ...
    expect_equal(result_meta$nirs_channels,
                 original_meta$nirs_channels)
    expect_equal(result_meta$time_channel,
                 original_meta$time_channel)
})
