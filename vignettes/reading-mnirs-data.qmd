...
* ⏱️ Resample data to a higher or lower sample rate, to correct irregular sampling periods, or match the frequency of other data sources for synchronisation.
...
*`mnirs`* data can be easily viewed by calling `plot()`. This generic plot function uses *`ggplot2`* and will work on data frames generated or read by *`mnirs`* functions where the metadata contains `class = *"mnirs"*`.
