...
Channels can also be specified and renamed explicitly. Since we have already read the file, we can instead modify channels using `create_mnirs_data()`. We will specify the raw haemoglobin concentration signals from the first NIRS channel, with the event column containing manually entered labels.
